...
for p in P:
    if Y[p,t].x > 0:
        print(f"| {str(p):<10} | {int(Y[p,t].x):<10} | {int(pipe_capacity * (1 + PU[p].x)):<12} | {str(int(PU[p].x)):<10} | {str(int(pipeline_cost * PU[p].x * P[p])):<10} |")

print("Pipeline total cost: $", (sum(PU[p].x * pipeline_cost * P[p] for p in P)), sep='')
