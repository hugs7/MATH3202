...
for n in N:
    for t in T:
        m.addConstr(X[(n,t)] + quicksum(Y[(p,t)] for p in P if p[1] == n) == 
                    demand[n][t] * (1 + change[t]) + quicksum(Y[(p,t)] for p in P if p[0] == n))
...
print("Pipeline total cost: $", (sum(pipeline_cost * (1 - discount[t]) * PU[p, t].x * P[p] for p in P for t in T)), sep='')
