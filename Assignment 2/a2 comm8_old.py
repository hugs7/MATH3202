...
for p in P:
    PU[p] = m.addVar(vtype=GRB.BINARY)

# Objective
m.setObjective(quicksum(
    quicksum(upgrade_costs[n][u] * Z[(n, u)] for n in N if n in upgrade_costs)
    for u in U) + quicksum(pipeline_cost * PU[p] * P[p] for p in P), 
               GRB.MINIMIZE)
...
for p in P:
    for t in T:
        m.addConstr(Y[(p,t)] <= (1 + PU[p]) * pipe_capacity) # MJ / day limit
...
for t in T:
    ...
    C = 290
    # Print pipes
    print(f"| {'Pipe (x,y)':<15} | {'Gas Flow':<15} | {'Maximum flow':<15}")
    for p in P:
        if Y[p,t].x > 0:
            print(f"| {str(p):<15} | {int(Y[p,t].x):<15} | {int(pipe_capacity * (1 + PU[p].x)):<15} |")
    ...
    for p in P:
        if Y[p,t].x > 0:
            print(f"| {str(p):<15} | {str(int(PU[p].x)):<15} | {str(int(PU[p].x * pipeline_cost)):<15} |")
    
    print("Pipeline total cost: $", int(sum(PU[p].x * pipeline_cost for p in P)), sep='')
