...
@author: Hugo Burton
s4698512
MATH3202 Assignment 3 - Integer Programming
Pacific Paradise Gas
Due 24/04/2023
Report attached
"""
...
# Comm 10 upgrade constraint

for s in S:
    m.addConstr(((quicksum(upgrade_costs[n][u] * Z[n, 1, u, s] for n in N if n in upgrade_costs for u in U))
                + 
                (quicksum(pipeline_cost * PU[p, 1, s] * P[p] for p in P)))
                <= 2 * 
                ((quicksum(upgrade_costs[n][u] * Z[n, 0, u, s] for n in N if n in upgrade_costs for u in U))
                + 
                (quicksum(pipeline_cost * PU[p, 0, s] * P[p] for p in P))))
...
for s in S:
    ...
    print("Pipeline total cost: $", (sum(pipeline_cost * (1 - discount[t]) * PU[p, t, s].x * P[p] for p in P for t in T)), sep='')
    

print("Overall total expected upgrade cost $" + str(m.objVal))
