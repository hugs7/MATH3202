...
def LocalTime(p, i, j):
    return sum(Time[i][k]*Freq[p[i]][p[k]]+
        ...
        Time[k][j]*Freq[p[k]][p[j]] for k in A) \
        - Time[i][j]*Freq[p[i]][p[i]] - \
        Time[i][j]*Freq[p[i]][p[j]] - \
        Time[j][i]*Freq[p[j]][p[i]] - \
        Time[j][i]*Freq[p[j]][p[j]]
...
def ChooseTwo(p):
    i, j = random.sample(A, 2)
    c1 = Time[i][i]*Freq[p[i]][p[i]] + Time[j][j]*Freq[p[j]][p[j]]
    c1 += Time[i][j]*Freq[p[i]][p[j]] + Time[j][i]*Freq[p[j]][p[i]]

    p[i], p[j] = p[j], p[i]

    c2 = Time[i][i]*Freq[p[i]][p[i]] + Time[j][j]*Freq[p[j]][p[j]]
    c2 += Time[i][j]*Freq[p[i]][p[j]] + Time[j][i]*Freq[p[j]][p[i]]
...
best = RunSA(startp, TotalTime, ChooseTwo, MoveTo, 10**3, 5*10**6, 0.999)
