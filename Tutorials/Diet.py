...
m.setObjective(quicksum(C[i] * x[i] for i in range(len(Foods))))

# Constraints

for i, f in enumerate(Nutrients):
    m.addConstr(quicksum(NV[j][i] * x.get(j) for j in range(len(Foods))) >= DMIN[i])
    if DMAX[i] != -1:
        m.addConstr(quicksum(NV[j][i] * x.get(j) for j in range(len(Foods))) <= DMAX[i])

# I want at least 150 g Apples per day
m.addConstr(x[1] >= 1.5)
...
for i, f in enumerate(Foods):
    if x.get(i).x >0:
        print(f, round(100 * x.get(i).x),2)
        
print("\nNutrients")
for i, n in enumerate(Nutrients):
    print(n, DMIN[i], DMAX[i], round(sum(NV[f][i] * x.get(f).x for f in range(len(Foods))),3))
