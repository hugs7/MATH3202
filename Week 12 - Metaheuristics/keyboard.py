...
# Begin with initial solution. Can be random but let's start with Querty
Sol = [ord(l)-65 for l in qwerty]

# Sol is now a vector of numbers instead of a vector of characters.

# Cost function is just total time.


def LocalTime(p, i, j):
    # To avoid having to use function TotalTime which is O(n^2)
    return sum(Time[i][k] * Freq[p[i]][p[k]] + 
               Time[j][k] * Freq[p[j]][p[k]] + 
               Time[k][i] * Freq[p[k]][p[i]] + 
               Time[k][j] * Freq[p[k]][p[j]]
               for k in A) \
               - Time[i][i]*Freq[p[i]][p[i]] - Time[j][j]*Freq[p[j]][p[j]] \
...
def ChooseTwo(Sol):
    ...
    # Use rand
    i,j = random.sample(A,2)        # Choose two positions out of the alpabet

    # calculate cost difference
    c1 = LocalTime(Sol, i, j)     # Current cost
    Sol[i],Sol[j] = Sol[j], Sol[i]  # Swap letters in i,j position in solution
    c2 = LocalTime(Sol, i, j)
    Sol[i],Sol[j] = Sol[j], Sol[i]  # Swap back letters in i,j position in solution
    return c2-c1, (i,j) # delta in cost and what neighbour is (i,j)
        
    
# Move to Neigh
# Implement move returned from Choose Neigh above :)
def MoveTwo(Sol, Neigh):
    i,j = Neigh
...
NewSol = RunSA(Sol, TotalTime, ChooseTwo, MoveTwo, T, N, alpha)
