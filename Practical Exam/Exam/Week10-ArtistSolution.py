...
def artist(i,S):
    ...
    # general case
    return max(
        (Price*Sales[j] - costs[i][j] + artist(j,S+[j])[0], j)
        for j in Cities if j not in S)

# Optimal solution is H -> B -> A -> G -> Home for expected profit of $2364
...
def artist2(i,S,n):
    ...
    return max(
        ( - costs[i][j] + 
         sum(p[j][k]*(Price*min(n,k) + artist2(j,S+[j],n-min(n,k))[0]) for k in K), j)
        for j in Cities if j not in S)
